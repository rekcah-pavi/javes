          ...
          try:                
              ...
              if response2.text.startswith("🔸"):      
                 ...
                 if response.text.startswith("🔸"):               
                    await event.edit(f"`{JAVES_NNAME}`: **Sorry, Failed to decode this img \n try use alternative way !ss3**")
                 else: 
...
async def _(event):
    ...
    if reply_message.sender.bot:
       await event.edit(f"`{JAVES_NNAME}`: **Reply to actual users message**")
       return
    await event.edit(f"`{JAVES_NNAME}`: ** Processing user chat to Sticker **")
    async with bot.conversation(chat) as conv:
          try:                
              await conv.send_message("/start")
              response2 = await conv.get_response()     
              if response2.text.startswith("**Hi!**"): 
                 await asyncio.sleep(0.3)
                 #await event.delete()
